...
function run() {
    ...
    echo "Start Command: nohup java $JAVA_MEM_OPTS -jar $appDir/$appName --spring.profiles.active=$profiles --server.port=$serverPort >> $appStartLog 2>&1 &\n"
    nohup java $JAVA_MEM_OPTS -jar $appDir/$appName --spring.profiles.active=$profiles --server.port=$serverPort >> $appStartLog 2>&1 &
}
